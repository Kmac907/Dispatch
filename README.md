...
Live endpoint validation requires at least one user-approved reachable Windows endpoint. Named lab devices such as `82H9704` and `92H9704` are validation targets, not permanent product requirements; if one supplied device is offline, powered down, disconnected, or unreachable, report that device as an environment availability issue and validate against another approved reachable endpoint. See `docs/testing-and-validation.md`.
