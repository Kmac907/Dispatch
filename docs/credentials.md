...
| `pscredential` | PowerShell module wrapper handoff only. | Invalid from direct `dispatch.exe` without protected wrapper handoff; module wrappers use supplied `-Credential` or prompt with `Get-Credential` when `-Credential` is omitted. |
...
`provider: pscredential` is reserved for PowerShell module execution wrappers, not ordinary direct `dispatch.exe` use. The module exposes diagnostic/version wrappers plus `Invoke-DispatchPowerShell`, `Invoke-DispatchCommand`, `Invoke-DispatchExecutable`, and `Invoke-DispatchJob` with `-CredentialName <name>` passthrough to the existing CLI `--credential <name>` reference. When the selected reference uses `provider: pscredential`, the module accepts optional `-Credential <PSCredential>`; if omitted, it prompts with `Get-Credential`, using the configured username when present. The wrapper passes the credential through a short-lived DPAPI CurrentUser-protected handoff file referenced only through child-process environment, then deletes the handoff file. Direct `dispatch.exe --credential <name>` rejects `pscredential` unless a compatible protected wrapper handoff is present. `provider: prompt` stays Dispatch-owned, so the module lets Dispatch perform the prompt instead of prompting itself.
