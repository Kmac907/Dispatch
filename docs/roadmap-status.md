...
- `hosts` command group has current `list`, `validate`, and transport-probe `test`; `graph` and `vars` remain planned.
...
- Continue `6.6` remaining hosts graph/vars subcommands and transport-aware doctor command surfaces.
