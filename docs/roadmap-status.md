...
- PowerShell module import with `Test-Dispatch`, `Get-DispatchVersion`, `Invoke-DispatchPowerShell`, `Invoke-DispatchCommand`, and `Invoke-DispatchExecutable` wrappers over the canonical CLI.
...
- PowerShell module execution wrappers are partial. Current module support includes diagnostics/version wrappers plus `Invoke-DispatchPowerShell`, `Invoke-DispatchCommand`, and `Invoke-DispatchExecutable` over `dispatch run ps|cmd|exe`, including `-CredentialName` passthrough to CLI `--credential`. Bundled-exe packaging, the job execution wrapper, and protected `PSCredential` handoff remain in Roadmap `7`.

## Planned Next

- Continue PowerShell module wrapper work with the job execution wrapper, bundled-exe packaging, and protected credential handoff.
