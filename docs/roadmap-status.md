...
- `dispatch run` execution results, completed `dispatch apply` execution for supported `ps`, `cmd`, and `exe` tasks, and completed `dispatch push` transfer/execute results return stable process exit codes for current result categories: `0` success, `2` host/execution failure, `3` probe/unreachable/timeout, `4` authentication/authorization, `5` transport unavailable, `6` cancelled, and `10` internal error. Usage/config/inventory/YAML/planning errors remain `1`.
...
- Broader command-family exit-code alignment outside the current `run`, executed `apply`, and completed `push` paths, plus full redaction validation, remain planned under `6.7`.

## Planned Next

- Finish remaining `6.7` CLI safety and policy items outside the current `dispatch run` execution-result exit-code mapping, `dispatch apply` executed-task exit-code preservation, completed `dispatch push` result exit-code mapping, LocalSystem policy gate, and PsExec fallback approval gate.
