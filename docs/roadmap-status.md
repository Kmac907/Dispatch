...
- Broader command-family exit-code alignment outside the current `run` execution and `apply` executed-task paths, plus full redaction validation, remain planned under `6.7`.

## Planned Next

- Finish remaining `6.7` CLI safety and policy items outside the current `dispatch run` execution-result exit-code mapping, `dispatch apply` executed-task exit-code preservation, LocalSystem policy gate, and PsExec fallback approval gate.
