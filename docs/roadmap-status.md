...
- PowerShell module import with `Test-Dispatch`, `Get-DispatchVersion`, and `Invoke-DispatchPowerShell` wrappers over the canonical CLI.
...
- PowerShell module execution wrappers are partial. Current module support includes diagnostics/version wrappers and `Invoke-DispatchPowerShell` over `dispatch run ps`, including `-CredentialName` passthrough to CLI `--credential`. Bundled-exe packaging, command/job execution wrappers, and protected `PSCredential` handoff remain in Roadmap `7`.

## Planned Next

- Continue PowerShell module wrapper work with command/job execution wrappers, bundled-exe packaging, and protected credential handoff.
