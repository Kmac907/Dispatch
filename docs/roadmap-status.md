...
- `doctor` supports local transport-scoped prerequisite checks, stable output modes, global Dispatch config parseability, credential provider availability, run-history layout checks, and PsExec local policy/EULA diagnostics, but host-schema diagnostics and some broader checks remain planned.
