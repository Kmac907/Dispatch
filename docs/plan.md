...
Current implementation note:
- `run ps` supports `-i|--inventory`, `-t|--target`, and `--exclude` for direct host selectors, simple text inventories, a small YAML inventory subset, groups, nested `groups.<group>.children`, list-form and mapping-form group members under `groups.<group>.hosts`/`children`, host names, `tag:<name>`, and `file:<path>`.
