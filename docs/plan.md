...
- Config-defined `credentials:<name>` entries are exposed to credential commands, including `provider: prompt` no-enrollment behavior for `creds add`, `provider: dpapi_file` protected-file enrollment/test/remove behavior, and `provider: windows_credential_manager` target enrollment/test/remove behavior.
...
- For real `--transport psrp` execution, `provider: prompt`, `provider: dpapi_file`, and `provider: windows_credential_manager` credential references resolve after planning and before live rendering or endpoint work; the password stays in memory, is passed to PSRP command/script/artifact sessions as a `PSCredential`, and is disposed after the run. Dry-run/plan paths validate and carry the reference name but do not prompt, decrypt DPAPI files, or read Windows Credential Manager targets.
...
- PSCredential PowerShell-wrapper handoff, Azure Key Vault resolution, raw WinRM credential handoff, PsExec credential handoff, DPAPI protected-file ACL hardening, and job-file credential validation remain in later slices. Job-file validation depends on Roadmap `6.5` introducing the YAML job parser.
