...
- `dispatch doctor --transport auto|psexec|psrp|winrm` is implemented for local transport-scoped prerequisite checks. Roadmap 6.8 adds stable doctor output modes, run-history layout checks, PsExec local policy/EULA diagnostics, and continues broader diagnostics migration work.
