...
- `dispatch run` maps completed execution results to stable process exit codes for success, host failure, probe/timeout, authentication/authorization, transport unavailable, cancellation, and internal error outcomes. Completed `dispatch apply` execution preserves those same stable underlying run exit codes for executed `ps`, `cmd`, and `exe` tasks. Usage/config/inventory/YAML/planning validation still returns `1`. Policy failures, including LocalSystem policy failures and PsExec fallback approval failures, now return plan/check policy exit code `7` before planning or endpoint work. Broader command-family exit-code alignment outside the current `run` execution and `apply` executed-task paths, plus full redaction validation, remain Roadmap `6.7` work.
