...
- `Dispatch.Transports.WinRm` is a partially implemented raw WinRM transport with current request-validation, planning, DI, endpoint-probe coverage, and chunked script-transfer preparation planning; WS-Management shell/command execution remains roadmap work.
...
winrm  + ScriptPayload   = partial; planning, probe, and chunked script-transfer preparation planning implemented, explicit not-yet-implemented shell execution failure remains
