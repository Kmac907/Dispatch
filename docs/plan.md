...
- `Dispatch.Transports.WinRm` is a partially implemented raw WinRM transport with current request-validation, planning, DI, endpoint-probe coverage, chunked script-transfer preparation planning, remote script upload, and raw-shell-backed PowerShell script execution; direct command execution and artifact collection remain roadmap work.
...
winrm  + ScriptPayload   = partial; planning, probe, chunked script-transfer preparation planning, remote upload, and raw-shell-backed PowerShell script execution implemented; command execution, artifact collection, and live validation remain
