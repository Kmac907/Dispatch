...
Status: partial/current. Plan, check, and execution are implemented for selected multi-task script-first `ps`, scalar `cmd`, and scalar `exe` jobs in YAML order. Additional task types remain planned v1.
...
Scalar `job.vars` entries are passed to selected `ps` tasks as named PowerShell script arguments in YAML order. Scalar `cmd` tasks run through the same command payload path as `dispatch run cmd`, and scalar `exe` tasks run through the same command payload path as `dispatch run exe`. Inventory vars remain host/group metadata and do not become runtime task inputs.
...
Use `--tags <tags>` to select `ps`, `cmd`, and `exe` tasks when at least one task tag matches. Use `--skip-tags <tags>` to exclude supported tasks when any task tag matches. Tag values are comma-separated, and a filter that excludes every supported task fails before endpoint work. Execution runs the selected `ps`, `cmd`, and `exe` tasks in YAML order and stops after the first failed task run.
