...
Script secret handoff is planned first for `dispatch run ps ... --secret name=reference`. It is not part of the current `apply` subset. When job-level script secrets are added, they must follow the same redacted script-parameter model and no-secret-values-in-logs rule.
