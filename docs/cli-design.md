...
Accepted transport names are broader than the currently completed executors. `psrp` is now accepted by request validation, registered into the host, covered by a preflight WSMan reachability probe, and can execute direct command payloads through a remote runspace with successful elevated live validation against `82H9704` and `92H9704`; script payloads still return an explicit structured not-yet-implemented execution failure until the remaining PSRP runspace path lands. `winrm` is now a completed execution path that can validate requests, build plans, probe endpoint reachability, upload a prepared script, execute that prepared PowerShell script, execute direct command payloads, classify shell timeouts plus shell-open authentication/authorization/transport failures into the shared result model, and collect declared/default artifact folders. Elevated live validation now succeeds from the current admin workstation against `82H9704` and `92H9704`.
