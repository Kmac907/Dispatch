...
Current roadmap priority is `10` Script-Owned Payload Documentation And Guardrails. Roadmap `7` PowerShell Module Wrapper is complete: the module imports and exposes `Test-Dispatch`, `Get-DispatchVersion`, `Invoke-DispatchPowerShell`, `Invoke-DispatchCommand`, `Invoke-DispatchExecutable`, and `Invoke-DispatchJob` over the canonical CLI, supports protected wrapper `PSCredential` handoff for configured `provider: pscredential` references, and `packaging/build-module.ps1` assembles the module with bundled `bin\win-x64\dispatch.exe`. Roadmap `8` source install and local packaging is complete for v1: source install, assembled-package install, direct PATH invocation for `dispatch`, optional ZIP creation, package validation, and temporary source cleanup status reporting are implemented. PsExec, raw WinRM, and PSRP are the three current transport paths today. PSRP now has shared-path slices where `psrp` requests validate for script and command payloads, the host registers PSRP transport services, preflight performs DNS plus default WSMan port reachability probing, direct command payloads execute through a PowerShell remote runspace, PowerShell scripts execute through a runspace-backed remote script path, declared/default artifact folders collect back over the remoting channel, structured `streamRecords` capture PSRP warning/verbose/debug/information/error output for the current transport path, optional shared execution-context configuration-name selection defaults to `Microsoft.PowerShell`, and the shared execution context explicitly includes later Kerberos and CredSSP placeholders while the implemented runtime still accepts only WSMan plus current-user `Default` or `Negotiate`. Unsupported future auth/connection selections fail during request validation instead of being ignored. Current PSRP failures classify timeout conditions into `TimedOut` and preserve consistent transport metadata on both success and failure, and elevated live validation for both current execution paths succeeds against locally approved validation hosts. Raw WinRM now covers request validation, planning, DI registration, endpoint reachability probes, chunked script-transfer preparation planning, remote script upload without SMB/admin shares, raw-shell-backed PowerShell script execution, direct command execution, shell-open authentication/authorization/transport classification, timeout classification in the shared result model, and artifact collection over the WinRM channel, with successful elevated live validation against locally approved validation hosts. Current `dispatch run` execution results, completed `dispatch apply` execution for supported `ps`, `cmd`, and `exe` tasks, completed `dispatch push` transfer/execute results, and `dispatch hosts test` endpoint-probe results return stable process exit codes for implemented result categories. Usage/config/inventory/YAML/planning and local lifecycle/inspection command failures remain exit code `1`. LocalSystem and PsExec fallback policy failures return plan/check policy exit code `7` before planning or endpoint work. Dispatch-owned rich/table output, structured JSON/NDJSON/YAML output, durable `Admin\events.ndjson`, durable `Admin\results.json`, optional per-target `result.json`, optional `Admin\results.csv`, and optional `Admin\dispatch.log` redact secret-looking values before writing. Raw `stdout.txt`, raw `stderr.txt`, and copied artifact content remain script-authored output, so scripts must avoid printing or writing secrets there.
...
```text
Invoke-DispatchPowerShell  # current; calls dispatch.exe run ps ...
Invoke-DispatchCommand     # current; calls dispatch.exe run cmd ...
Invoke-DispatchExecutable  # current; calls dispatch.exe run exe ...
Invoke-DispatchJob         # current; calls dispatch.exe apply ...
Test-Dispatch              # current; calls dispatch.exe doctor
Get-DispatchVersion        # current; calls dispatch.exe version
```
...
Omitted `--transport` and `--transport auto` must not implicitly select PsExec unless fallback policy is approved. Explicit `--transport psexec` remains the CLI opt-in and does not depend on fallback approval. Config approval is `dispatch.allow_psexec_fallback: true`. Inventory approval can be `allow_psexec_fallback: true` on defaults, group vars, host entries, or host vars where that location is supported by the current implementation. Missing approval is a policy failure and returns exit code `7` before planning or endpoint work.
...
Script secret handoff is separate from credential-reference precedence. `--credential <name>` authenticates to the endpoint. The `dispatch run ps ... --secret name=reference` surface selects script input secrets and maps `name` to a script parameter. Current support validates the option shape and renders redacted parameter bindings such as `-packageSas [redacted]` in plan/dry-run output without resolving or printing secret values. Roadmap 10 owns real execution: Dispatch must resolve `reference` from a configured provider on the admin side and bind the resolved value through the selected transport without exposing it through command-line, console, log, result, trace, artifact, or process-inspection surfaces.
...
Current `copy` support is preview-only. `dispatch apply <job.yml> --plan` and `--check` accept block or inline mapping syntax with `src`, `dest`, and optional `overwrite`, resolve relative `src` paths from the job file directory, require the local source file to exist for selected preview tasks, require `dest` to be a rooted Windows path, apply task tags before validation, and render source, destination, overwrite, selected targets, and transport in the apply task envelope. `dispatch apply <job.yml>` rejects selected `copy` tasks before endpoint planning because file transfer execution is not implemented in this slice.

## Safety Rules

- Do not allow plaintext passwords in jobs, inventories, config, logs, or command-line flags.
- Do not log secrets.
- Do not enable PsExec fallback silently; omitted transport and `--transport auto` require `dispatch.allow_psexec_fallback: true` or supported inventory `allow_psexec_fallback: true` approval before they can resolve to PsExec.
...
7   Check/plan policy validation failed, including missing PsExec fallback approval.
10  Internal/unhandled error.
```

The stable result exit-code contract applies to `dispatch run`, executed `dispatch apply` tasks, completed `dispatch push` transfer/execute results, and `dispatch hosts test` endpoint-probe results. Usage/config/inventory/YAML/planning and local lifecycle/inspection command failures remain exit code `1`. Dispatch-owned output redaction is implemented for console summaries, structured output, durable event/result JSON, optional CSV, and optional text logs.
