...
The stable result exit-code contract applies to `dispatch run`, executed `dispatch apply` tasks, completed `dispatch push` transfer/execute results, and `dispatch hosts test` endpoint-probe results. Usage/config/inventory/YAML/planning and local lifecycle/inspection command failures remain exit code `1`. Remaining Roadmap `6.7` work is redaction validation.
