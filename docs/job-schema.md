...
Status: partial/current. `dispatch apply <job.yml> --plan`, `dispatch apply <job.yml> --check`, and `dispatch apply <job.yml>` currently support selected multi-task script-first `ps` and scalar `cmd` jobs in YAML order.
...
- selected `tasks` entries using `ps` or `cmd`
- `hosts`
- `transport`
- `credential`
